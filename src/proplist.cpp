    ...
    Type PropertyList::get##TypeName(const std::string &name) const { \
        ...
        if (it->second.type != Property::TypeName##_type) \
            throw NoriException("Property '%s' has the wrong type! " \
                "(expected <" #XmlName ":" #TypeName ">)!", name); \
...
DEFINE_PROPERTY_ACCESSOR(Point3f, Point3, point)
DEFINE_PROPERTY_ACCESSOR(Vector3f, Vector3, vector)
DEFINE_PROPERTY_ACCESSOR(Point2f, Point2, point)
DEFINE_PROPERTY_ACCESSOR(Vector2f, Vector2, vector)
